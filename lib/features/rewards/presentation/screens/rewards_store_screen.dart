...
class _RewardsStoreScreenState extends State<RewardsStoreScreen>
  ...
  void _loadRewards() async {
    ...
    setState(() {
      _rewards = _rewardsRepository.allRewards;
      _isLoading = false;
    });
  }
  ...
  Widget _buildPointsBalance() {
    final userRewards = _rewardsRepository.currentUserRewards;
  ...
  Widget _buildRewardCard(Reward reward) {
    final userPoints = _rewardsRepository.currentUserRewards?.totalPoints ?? 0;
...
class _RewardDetailsBottomSheet extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final userPoints = rewardsRepository.currentUserRewards?.totalPoints ?? 0;
    final canAfford = userPoints >= reward.pointsCost;
    final canRedeem = reward.canRedeem && canAfford;
  ...
  Future<void> _redeemReward(BuildContext context) async {
    try {
      await rewardsRepository.redeemReward(reward.id);
